...
def create_bucket(bucket_name, region=None):
    ...
    If a region is not specified, use the active boto3 session region.
    ...
    effective_region = region or boto3.session.Session().region_name or "us-east-1"
    s3_client_for_bucket = boto3.client(
        's3', region_name=effective_region
    )

    try:
        s3_client_for_bucket.head_bucket(Bucket=bucket_name)
    ...
    try:
        if effective_region == "us-east-1":
            resp = s3_client_for_bucket.create_bucket(Bucket=bucket_name)
        else:
            location = {'LocationConstraint': effective_region}
            resp = s3_client_for_bucket.create_bucket(
                Bucket=bucket_name,
        ...
        logging.error(e)
        return False
    s3_client_for_bucket.put_bucket_tagging(
        Bucket=bucket_name,
        Tagging={
            "TagSet": [
                {"Key": "workshop-kb", "Value": "true"},
...
def bucket_exists(bucket_name):
    ...
    try:
        ...
        if e.response["Error"].get("Code") in {"404", "NoSuchBucket", "NotFound"}:
            return False
        else:
